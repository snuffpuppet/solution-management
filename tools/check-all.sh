...
cd "$(dirname "$0")/.."
fail=0
for s in tools/check-model-prc.sh tools/check-model-cr.sh tools/check-runner.sh tools/check-transcript-runner.sh tools/check-stakeholders.sh tools/check-roles.sh tools/test-vtt-to-passages.sh tools/test-ingestion-report.sh tools/test-check-registers.sh tools/check-registers.sh; do
  echo "== $s"; "$s" || fail=1
done
echo "== README"
...
grep -qF 'registers/' .claude/skills/ingest-transcript/SKILL.md && echo "ok   ingest skill chains the register runner" || { echo "FAIL ingest skill chains the register runner"; fail=1; }
echo "== gitignore"
...
echo "== skills"
for s in ingest-transcript build-registers ingestion-report handover; do grep -q "^name: $s$" ".claude/skills/$s/SKILL.md" 2>/dev/null && echo "ok   skill $s" || { echo "FAIL skill $s"; fail=1; }; done
echo "== handover"
