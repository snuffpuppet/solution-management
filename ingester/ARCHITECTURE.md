...
A set of playbook-style markdown documents, plus a few shell tools, for managing solution architecture registers on a vendor-delivered project and for feeding those registers from discovery-session transcripts. Two AI runners execute the documents inside Claude Code, one human at a time, with a checkpoint at every stage. The registers themselves live in an engagement as markdown tables, so the project is self-contained; a Confluence target exists for later integration. The repository holds one ingester and any number of engagements: `ingester/` is the method, and each `engagements/<name>/` is one engagement's data.

| Layer | File | Responsibility |
|---|---|---|
| Model | `<HOW>/solution-register-model.md`, `<HOW>/roles.md` | What items are: types, states, fields, relationships, views, integrity rules. Who speaks in a session and what each role may yield. Tool-agnostic. |
...
P7. **Separate the transcript runner from the lifecycle.** The model can be used without the transcript runner, and the transcript runner can change without touching the model. The only coupling is the claim record's class and relation vocabulary.
...
P10. **A check for every rule that matters.** When a document gains a rule another document or tool depends on, `<HOW>/tools/check-all.sh` gains an assertion for it in the same commit.
...
### D11. Registers are built locally, and every ingestion ends with them
Added 9 September 2026 (version 1.10). The register runner's default target is `registers/` in the engagement, one markdown table per register with the model's columns; Confluence is a second target chosen at Phase 0. Rows already in a register are the record: a run reads them first, continues ids from them, appends new rows and changes an existing row only through an approved before and after pair in the dry run. `/ingest-transcript` runs the register runner on the local target after T4, so claims never sit unregistered. The deterministic part of the integrity rules is `<HOW>/tools/check-registers.sh`, which also fails when a register header drifts from model section 7. Consequence: the project distils requirements, processes, systems and decisions from transcripts end to end without any external system.
...
A finding about the method, discovered while working in an engagement, goes in that engagement's `FINDINGS.md`, and an ingester session promotes it into `<HOW>/ENHANCEMENTS.md` with an id; ids are issued only by the ingester, so two engagements cannot collide. Consequence: two people can work on the method and on an engagement at once, and the method can be read by a second engagement without being copied.
...
[Register runner]  Phase 0 preflight -> 1 inventory -> 2 classify and extract -> 3 build -> 4 reconcile -> 5 handover
      |
      v
registers/ (default): eight registers, taxonomy, outstanding and next-phase views
   or the Confluence design register folder, when that target is chosen
```

Design documents enter the knowledge base by a separate ingestion outside this repository. Transcripts enter through the transcript runner. Both meet at the claim. The `/ingest-transcript` skill chains the two runners, so one ingestion ends with the session's claims and the updated registers both committed; `/build-registers` runs the register runner alone.
...
| Register Phase 3a | Dry run of every register file or page, with updated rows shown before and after | Writing to `registers/` or to Confluence |
| Register Phase 4 | Each source page edit, individually (Confluence only) | That one edit |
...
- `HANDOVER.md`, a transient statement of where the last session stopped, rewritten by the `handover` skill when the user ends a session and verified by `<HOW>/tools/handover-check.sh`. Enforcement as a Stop hook is optional and per user.
...
5. On PROCEED, run `<HOW>/tools/check-all.sh` before and after, and add assertions for any new rule (P10).
